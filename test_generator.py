...
def test(opt, test_loader, tocg, generator):
    ...
    with torch.no_grad():
        for inputs in test_loader.data_loader:
            ...
            if opt.cuda :
                pose_map = inputs['pose'].cpu()
                pre_clothes_mask = inputs['cloth_mask'][opt.datasetting].cpu()
                label = inputs['parse']
                parse_agnostic = inputs['parse_agnostic']
                agnostic = inputs['agnostic'].cpu()
                clothes = inputs['cloth'][opt.datasetting].cpu() # target cloth
                densepose = inputs['densepose'].cpu()
                im = inputs['image']
                input_label, input_parse_agnostic = label.cpu(), parse_agnostic.cpu()
                pre_clothes_mask = torch.FloatTensor((pre_clothes_mask.detach().cpu().numpy() > 0.5).astype(np.float32)).cpu()
            else :
            ...
            if opt.cuda :
                warped_cm_onehot = torch.FloatTensor((warped_clothmask_paired.detach().cpu().numpy() > 0.5).astype(np.float32)).cpu()
            else :
            ...
            if opt.cuda :
                old_parse = torch.FloatTensor(fake_parse.size(0), 13, opt.fine_height, opt.fine_width).zero_().cpu()
            else:
            ...
            if opt.cuda :
                parse = torch.FloatTensor(fake_parse.size(0), 7, opt.fine_height, opt.fine_width).zero_().cpu()
            else:
